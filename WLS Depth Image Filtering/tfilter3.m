...
function B = tfilter3(A,w,sigma,nrm)
...
if size(A,3) == 1
   B = tfltGray(A,w,sigma(1),sigma(2),nrm);
else
   B = tfltColor(A,w,sigma(1),sigma(2),nrm);
end


%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Implements bilateral filtering for grayscale images.
function B = tfltGray(A,w,sigma_d,sigma_r,nrm)
...
% Create waitbar.
h = waitbar(0,'Applying trilateral filter...');
set(h,'Name','Trilateral Filter Progress');
...
for i = 1:dim(1)
   for j = 1:dim(2)
         ...
         N = nrm(iMin:iMax,jMin:jMax,:);
         %t=repmat(nrm(i,j),[iMax-iMin+1 jMax-jMin+1 1] ,3);
         t=repmat(nrm(i,j,:),[iMax-iMin+1 jMax-jMin+1 1]);
         O = exp(-real(acos(dot(N ,t , 3))).^2/(2*sigma_o^2));
         % Compute Gaussian intensity weights.
         H = exp(-(I-A(i,j)).^2/(2*sigma_r^2));
    
         % Calculate bilateral filter response.
         F = H.*G((iMin:iMax)-i+w+1,(jMin:jMax)-j+w+1).*O;
         B(i,j) = sum(F(:).*I(:))/(sum(F(:)));
         if(isnan(B(i,j))) B(i,j)=A(i,j);   end      
   end
   waitbar(i/dim(1));
end
...
function B = tfltColor(A,w,sigma_d,sigma_r,nrm)
...
% Create waitbar.
h = waitbar(0,'Applying triilateral filter...');
set(h,'Name','Trilateral Filter Progress');
