...
function [ ret ] = bilateral_me (im, filterSize, sSpatial, sIntensity,C)
...
for i=1:height
	...
	for j=1:width
		...
		% The center pixel
		center = im (i,j);
         if (mod(i,2)==0) ii=i/2;  else ii= floor(i/2)+1; end
           if (mod(j,2)==0) jj=j/2;  else jj= floor(j/2)+1; end
           cc=C(ii,jj);
           if (cc==1) area=center; end
        
        %% modified by taha
%         for m=xmin:xmax;
%             for n=ymin:ymax
%             
%             if (mod(m,2)==0) mm=m/2;  else mm= floor(m/2)+1; end
%             if (mod(n,2)==0) nn=n/2;  else nn= floor(n/2)+1; end
%                  cw=C(mm,nn);
%                  if (cc ~= cw)
%             end %j
%         end % i
